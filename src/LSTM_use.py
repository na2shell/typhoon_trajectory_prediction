...
def time_collate_fn(batch):
    ...
    traj_class_indices = torch.nn.utils.rnn.pad_sequence(traj_class_indices, batch_first=True, padding_value=111)
...
if train:
    for epoch in range(epoch_num):
        ...
        for data, traj_len, traj_class_indices in dataloader:
            ...
            # just for exp
            # data_for_D = data[:, :, :2]
            packed_data = torch.nn.utils.rnn.pack_padded_sequence(data, batch_first=True, lengths=traj_len, enforce_sorted=False)
            ...
            day_onehot = torch.nn.functional.one_hot(torch.argmax(day, dim=2), num_classes=7)
            hour_onehot = torch.nn.functional.one_hot(torch.argmax(hour, dim=2), num_classes=24)
            category_onehot = torch.nn.functional.one_hot(torch.argmax(category, dim=2), num_classes=10)

            fake_input = torch.cat([lat_lon, day_onehot, hour_onehot, category_onehot], dim=-1)
            # fake_input = lat_lon

            fake_input = fake_input.detach()
            packed_fake_input = torch.nn.utils.rnn.pack_padded_sequence(fake_input, batch_first=True, lengths=traj_len, enforce_sorted=False)
            ...
            fake_inputs = torch.cat([lat_lon, day_onehot, hour_onehot, category_onehot], dim=-1)
            # fake_inputs = lat_lon

            # lat, lng, day, hour, category = G(noise_data)

            packed_fake_inputs = torch.nn.utils.rnn.pack_padded_sequence(fake_inputs, batch_first=True, lengths=traj_len, enforce_sorted=False)
            ...
            G_loss_equation = torch.sqrt(
                mse_loss_with_mask(fake_inputs[:,:,0], data[:,:,0], ignored_index=99)
                + mse_loss_with_mask(fake_inputs[:,:,1], data[:,:,1], ignored_index=99)
            )
            
            day_loss = ce_loss(fake_inputs[:,:,2:9].view(-1,7), traj_class_indices[:,:,2].view(-1).to(torch.long))
            category_loss = ce_loss(fake_inputs[:,:,9:19].view(-1,10), traj_class_indices[:,:,3].view(-1).to(torch.long))
            hour_loss = ce_loss(fake_inputs[:,:,19:].view(-1,24), traj_class_indices[:,:,4].view(-1).to(torch.long))

            G_loss = G_loss_GAN + latlon_corr*G_loss_equation + day_loss + category_loss + hour_loss
        ...
        print("epoch :", epoch, sum(g_losses) / len(g_losses), sum(g_losses_bce) / len(g_losses_bce), sum(d_losses) / len(d_losses))
