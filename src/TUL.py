...
class TUL_attack_model(nn.Module):
    ...
    def forward(self, x, traj_len):
        ...
        # x = self.output_layer(seq_unpacked[:, -1, :])

        # x = self.transformer_encoder(x)
        # max_val, indices= torch.max(x, dim=1)
        # x = self.output_layer(max_val)
...
if __name__ == "__main__":
    ...
    for epoch in range(epoch_num):
        ...
        for data, traj_len, traj_class_indices, label in train_dataloader:
            # print(data.size())
            pred_y = model(data, traj_len)
    ...
    preds = []
    actuals = []
    for data, traj_len, traj_class_indices, label in test_dataloader:
        pred_y = model(data, traj_len)
